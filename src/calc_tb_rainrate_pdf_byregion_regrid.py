"""
Calculate histograms of coarsened OLR/Tb, total & MCS precipitation for a region and save output to a netCDF file.
"""
import numpy as np
import sys
import xarray as xr
...
if __name__ == "__main__":
    ...
    file_landmask = f'/pscratch/sd/f/feng045/DYAMOND/maps/era5_landmask.nc'
    print(f'OLR/precipitation file: {rain_file}')
    print(f'MCS mask file: {mask_file}')
    # Outputs 
    out_dir = f'/pscratch/sd/f/feng045/DYAMOND/mcs_stats/{PHASE}/{tracker}/'
    out_filename = f'{out_dir}tb_rainrate_hist_{PHASE}_{runname}_regrid.nc'
    ...
    # Read landmask data
    dslm = xr.open_dataset(file_landmask).sel(
    ...
    dsr_calendar = dsr['time'].encoding.get('calendar', None)
    # Convert 'noleap'/'365_day' calendar time to datetime to DatetimeIndex (e.g., SCREAM)
    # if dsr_calendar == 'noleap':
    if (dsr_calendar == '365_day') | (dsr_calendar == 'noleap'):
    ...
    ocean_mask = (landseamask >= min(ocean_thresh)) & (landseamask <= max(ocean_thresh))
    land_mask = (landseamask >= min(land_thresh)) & (landseamask <= max(land_thresh))
    mcspcp_o = ds['precipitation'].where((ds['mcs_mask'] > 0) & ocean_mask)
    mcspcp_l = ds['precipitation'].where((ds['mcs_mask'] > 0) & land_mask)
